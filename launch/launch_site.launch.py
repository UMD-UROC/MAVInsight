...
initial_paths_overrides = ["c2_c130_maponly.yaml"]
...
def build_nodes(paths: list[Path], global_config: Path) -> list[Node]:
    ...
    while paths:
        ...
        assert isinstance(config_path, Path), "Unrecognized build_nodes input type."
        ...
        if config_path in processed:
            ...
            continue
        LOGGER.debug("non-circular path")
        ...
        if abs_path is None:
            ...
            continue
        LOGGER.debug("abs path acquired")
        ...
        LOGGER.debug("Base yaml acquired")
        ...
        try:
            ...
            continue
        LOGGER.debug("File type identified")
...
def resolve_config_file(path: Path) -> Path | None:
    ...
    package_configs = (
        Path(get_package_share_directory(package_name)) / "package_resources"
    )
    resolved_path = package_configs / path
    if not resolved_path.is_file():
        raise FileNotFoundError(
            f"Could not find configs for: {path} in mavinsight configs folder."
        )
