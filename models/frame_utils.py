...
R_frd_flu = R.from_matrix(np.array([[1, 0, 0], [0, -1, 0], [0, 0, -1]]))

R_cam_frd = R.from_matrix(
    np.array(
        [
            [0, 0, 1],
            [1, 0, 0],
...
def frd_ned_2_flu_enu(input):
    if isinstance(input, Quaternion):
        r = R.from_quat([input.x, input.y, input.z, input.w])
        (x_out, y_out, z_out, w_out) = (
            R_ned_enu * r * R_frd_flu
        ).as_quat()  # NOTE: TF2 quaternions are child->parent
        return Quaternion(x=x_out, y=y_out, z=z_out, w=w_out)
    elif isinstance(input, R):
...
def enu_2_lla(ref: NavSatFix, e, n, u):
    return pm.enu2geodetic(e=e, n=n, u=u, lat0=ref.latitude, lon0=ref.longitude, h0=ref.altitude)
