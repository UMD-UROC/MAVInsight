...
class Sensor(GraphMember):
    ...
    def check_dict(self, config_params):
        ...
        if ("offset" not in config_params.keys()) or (
            len(config_params["offset"]) == 0
        ):
        ...
        if not set(Sensor.param_reqs).issubset(set(config_params.keys())):
            raise ValueError(
                f"Not enough params in dict to create Sensor. Must have all of: {Sensor.param_reqs}"
            )

        # parameter checking
        if len(config_params["offset"]) != 3:
            raise ValueError(
                f"Sensor offset must be exactly 3 elements. Received: {config_params['offset']}"
            )
        if any([not isinstance(val, float) for val in config_params["offset"]]):
            raise ValueError(
                f"Sensor offset values must be floats. Received: {config_params['offset']}"
            )
...
class Camera(Sensor):
    ...
    def check_dict(self, config_params):
        ...
        if not set(Camera.param_reqs).issubset(set(config_params.keys())):
            raise ValueError(
                f"Not enough params in dict to create Camera. Must have all of: {Camera.param_reqs}"
            )
...
class Gimbal(Sensor):
    ...
    def check_dict(self, config_params):
        ...
        if not set(Gimbal.param_reqs).issubset(set(config_params.keys())):
            raise ValueError(
                f"Not enough params in dict to create Gimbal. Must have all of: {Gimbal.param_reqs}"
            )
...
class Rangefinder(Sensor):
    ...
    def __init__(
        ...
        frame_name: str = None,
        name: str = None,
        offset: list[float, float, float] = [0.0, 0.0, 0.0],
        parent_frame: str = None,
    ...
    def check_dict(self, config_params):
        ...
        if not set(Rangefinder.param_reqs).issubset(set(config_params.keys())):
            raise ValueError(
                f"Not enough params in dict to create Rangefinder. Must have all of: {Rangefinder.param_reqs}"
            )
...
def sensor_factory(filename: str) -> Sensor:
    ...
    with open(path, "r", encoding="utf-8") as sensor_file:
        ...
        if type(sensor_config) is not dict:
            raise TypeError(
                f"Error parsing {path} as yaml in sensor_factory. Sensor configs must be yaml-encoded."
            )

        if "sensor_type" not in sensor_config:
            raise ValueError(
                f"Sensor config file {path} does not contain sensor type parameter."
            )
        ...
        match sensor_config["sensor_type"]:
            ...
            case _:
                raise ValueError(
                    f"Unrecognized Sensor type: {sensor_config['sensor_type']}"
                )
