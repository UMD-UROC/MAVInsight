...
class Site(GraphMember):
    ...
    def __init__(self):
        ...
        if self.has_parameter("geofence_topic"):
            self.GEOFENCE_TOPIC = (
                self.get_parameter("geofence_topic").get_parameter_value().string_value
            )
        else:
        ...
        if self.has_parameter("geofence"):
            geofence = (
                self.get_parameter("geofence").get_parameter_value().double_array_value
            )
        else:
            geofence = None
        ...
        if self.has_parameter("ground_truth_topic"):
            self.GT_TOPIC = (
                self.get_parameter("ground_truth_topic")
                .get_parameter_value()
                .string_value
            )
        else:
        ...
        if self.has_parameter("ground_truths"):
            ground_truths = (
                self.get_parameter("ground_truths")
                .get_parameter_value()
                .double_array_value
            )
        else:
        ...
        if self.has_parameter("map_ref_topic"):
            self.MAP_REF_TOPIC = (
                self.get_parameter("map_ref_topic").get_parameter_value().string_value
            )
        else:
        ...
        if self.has_parameter("map_ref"):
            map_ref = (
                self.get_parameter("map_ref").get_parameter_value().double_array_value
            )
        else:
            raise RuntimeError(
                f"Site: {self.DISPLAY_NAME}'s map param is not set. Unable to initialize site."
            )
        ...
        self.map_ref_pub = self.create_publisher(
            NavSatFix, f"/{self.NAME}{self.MAP_REF_TOPIC}", 1
        )
        ...
        if geofence:
            if len(geofence) % 2 != 0:
                raise ValueError(
                    "Non-even geofence list length. Ensure all lats and lons are paired.\n"
                    + f"length: {len(geofence)}"
                )
            self.geofence_pub = self.create_publisher(
                Marker, f"/{self.NAME}{self.GEOFENCE_TOPIC}", viz_qos
            )
            ...
            vertices = [
                lla_2_enu(self.map_lla_ref, NavSatFix(latitude=lat, longitude=lon))
