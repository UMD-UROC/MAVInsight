...
class Site(GraphMember):
    ...
    def __init__(self):
        ...
        if self.has_parameter('ground_truths'):
            self.ground_truths = (
                self.get_parameter('ground_truths').get_parameter_value().double_array_value
            )
        else:
        ...
        if self.has_parameter('local_fix_topic'):
            ...
            raise RuntimeError(
                f'Site Node: {self.DISPLAY_NAME} local fix param not set. Unable to initialize Site node.'
            )
        ...
        if self.has_parameter('local_frame'):
            ...
            raise RuntimeError(
                f'Site Node: {self.DISPLAY_NAME} local frame not set. Unable to initialize Site node.'
            )
    ...
    def site_foxglove_loiter(self):
        ...
        if self.geofence:
            ...
            vertices = [
                lla_2_enu(self.LOCAL_FIX, NavSatFix(latitude=lat, longitude=lon))
