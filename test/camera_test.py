...
class TestCamera(unittest.TestCase):
    ...
    def test_camera_from_yaml(self):
        ...
        self.assertRaises(
            ValueError, sensor_factory, os.path.join(self.cm_dir, "cm_no_name.yaml")
        )
        self.assertRaises(
            ValueError, sensor_factory, os.path.join(self.cm_dir, "cm_no_frame.yaml")
        )
        self.assertRaises(
            ValueError, sensor_factory, os.path.join(self.cm_dir, "cm_no_parent.yaml")
        )
        self.assertRaises(
            ValueError, sensor_factory, os.path.join(self.cm_dir, "cm_no_topic.yaml")
        )
        self.assertRaises(
            ValueError, sensor_factory, os.path.join(self.cm_dir, "cm_no_type.yaml")
        )

        # offset cases
        self.assertRaises(
            ValueError, sensor_factory, os.path.join(self.cm_dir, "cm_long_offset.yaml")
        )
        self.assertRaises(
            ValueError,
            sensor_factory,
            os.path.join(self.cm_dir, "cm_short_offset.yaml"),
        )
        self.assertRaises(
            ValueError,
            sensor_factory,
            os.path.join(self.cm_dir, "cm_none_in_offset.yaml"),
        )
        self.assertRaises(
            ValueError, sensor_factory, os.path.join(self.cm_dir, "cm_bad_offset.yaml")
        )
